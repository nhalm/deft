defmodule Deft.Eval.Spilling.SummaryQualityTest do
  ...
  describe "summary format - hard assertions" do
    test "grep summary contains cache:// reference", %{context: context} do
    ...
    test "grep summary mentions match count", %{context: context} do
      large_result = generate_grep_result(3000)
      summary = Grep.summarize([%Text{text: large_result}], "test-key-123")
    ...
    test "read summary contains cache:// reference", %{context: context} do
    ...
    test "read summary mentions line count", %{context: context} do
      large_result = generate_read_result(3000)
      summary = Read.summarize([%Text{text: large_result}], "test-key-456")
    ...
    test "ls summary contains cache:// reference", %{context: context} do
    ...
    test "ls summary mentions file/directory count", %{context: context} do
      large_result = generate_ls_result(3000)
      summary = Ls.summarize([%Text{text: large_result}], "test-key-789")
  ...
  defp judge_summary_quality(summary, full_result, tool_name) do
    # For now, use heuristic checks instead of actual LLM call
    # In production, this would call an LLM to judge quality

    # Check 1: Summary is significantly shorter
    summary_size = byte_size(summary)
    full_size = byte_size(full_result)
    size_reduction = summary_size < full_size * 0.3

    # Check 2: Contains key structural information
    has_counts = summary =~ ~r/\d+/
    has_cache_ref = summary =~ ~r/cache:\/\//

    # Check 3: Preview content is present (top N items)
    preview_lines = String.split(summary, "\n") |> Enum.count(fn line -> line != "" end)
    has_preview = preview_lines >= 3

    # Pass if all checks pass
    size_reduction and has_counts and has_cache_ref and has_preview
  end
end
