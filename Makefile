...
test.eval.check-structure:
	...
	@test $$(find test/eval -name "*.exs" -type f | wc -l) -ge 1 || \
		(echo "ERROR: test/eval/ is missing test files (found $$(find test/eval -name "*.exs" -type f | wc -l), expected at least 1)" && exit 1)
