...
- Define `@type run_id :: String.t()` and `@type category :: String.t()` in `Deft.Eval`. Update specs in `Eval.ResultStore`, `Eval.Baselines`, `Eval.JudgeCalibration`, and `Eval.RegressionDetection` to use them (blocked: Define model_name, provider_name, tool_name, tool_call_id, message id, job_id, lead_id, issue id types)
