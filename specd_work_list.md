...
- Add expandable tool calls: inline tool template in `chat_live.html.heex` has no click-to-expand details. Either use the existing `DeftWeb.Components.ToolCall` component or add expand logic inline, and add corresponding `handle_event("toggle_tool", ...)` in `ChatLive`.
