...
- Implement `Deft.OM.Observer.Parse.parse_output/1`: extract `<observations>` and `<current-task>` from XML; fallback to raw bullet-list extraction; validate section headers - Implement section-aware merge in State: Current State = replace, User Preferences/Decisions/Session History = append, Files & Architecture = append with dedup (same file path updates existing entry), unknown sections = ignore
