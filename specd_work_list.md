...
- Implement `cleanup/1` in Foreman: iterate `data.leads` map, call `GitJob.cleanup_lead_worktree/1` for each Lead with a worktree, `Process.exit(pid, :shutdown)` for each live Lead, demonitor all Leads with `:flush`, demonitor ForemanAgent, stop site log. This replaces the current stub that only stops site log.
...
- Add `match_contract_to_blocked_leads/2` private function in Foreman: takes contract metadata (lead_id, contract content) and `data`, checks `data.blocked_leads` for Leads whose dependency in `data.plan.dependencies` matches the publishing Lead, returns list of `{blocked_lead_id, lead_pid}` pairs to unblock. (blocked: Implement cleanup/1)
...
- Add `lead_message_buffer` (list) and `lead_message_timer` (timer ref or nil) fields to Foreman initial state data. Add `job.lead_message_debounce` config key with default 2000ms. (blocked: Implement cleanup/1)
...
- Add `job.lead_crash_decision_timeout` config key with default 60000ms. (blocked: Implement cleanup/1)
