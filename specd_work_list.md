...
- Fix `RateLimiter.start_link/1` to support per-job instances: currently registers with `name: __MODULE__` (global singleton); second concurrent job's RateLimiter will crash with `{:error, {:already_started, pid}}`; should use `{:via, Registry, ...}` registration keyed by job_id
...
- Fix CLI help text to show `--auto-approve-all`: moduledoc (cli.ex:23) and `print_help` (cli.ex:607) advertise `--auto-approve` but OptionParser (cli.ex:102) parses `--auto-approve-all`; user typing the documented flag gets an unknown flag error
