...
- Fix `:researching` state_enter: replace `{:next_event, :internal, :collect_research}` with `{:state_timeout, 0, :collect_research}` — next_event is prohibited in gen_statem state_enter callbacks, causes badreturn crash on entering `:researching` (foreman.ex:217)
