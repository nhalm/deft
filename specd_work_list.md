...
- Create missing invocation fidelity fixture files: `test/eval/fixtures/skills/multi_step_skill.json`, `test/eval/fixtures/skills/use_skill_tool.json`, `test/eval/fixtures/skills/ordered_steps_skill.json`; `invocation_fidelity_test.exs` line 51 pattern-matches `{:ok, fixture} = load_fixture(fixture_path)` which raises MatchError when files are missing
- Fix `scoring.ex` misclassification of `actor.continuation`: line 68 classifies it as `{:hard_assertion, 1.00}` but actor.md section 4.2 specifies 90% over 20 iterations (statistical); should be `{:statistical, 0.90}`
